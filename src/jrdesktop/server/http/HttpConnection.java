...
public class HttpConnection extends Thread {
    ...
    public void run() {
        try {
            ...
            while ((line = in.readLine()) != null) {
                ...
                if (line.startsWith("GET / ")) {
                ...
                else if (line.startsWith("GET /kadinio.html")) {
                    // send http headers
                    out.writeBytes("HTTP/1.0 200 \n");
                    out.writeBytes("Content-Type: text/html\n");
                    out.writeBytes("\n");
                    ...
                    // send page content
                    out.writeBytes(
                            "<html><head><title>Kadinio - Remote Desktop Viewer</title></head>" +
                            ...
                            "<applet code='kadinio.mainApplet.class' archive='http://" +
                            RMIServer.serverConfig.server_address + ":" +
                            RMIServer.serverConfig.http_port + "/kadinio.jar' " +
                    ...
                    out.writeBytes("Content-Disposition: attachment; filename=kadinio_applet.jnlp\n");
                ...
                else if (line.startsWith("GET /kadinio.jnlp")) {
                    String jnlp = getJNLP("http://" +
                            RMIServer.serverConfig.server_address + ":" +
                            String.valueOf(RMIServer.serverConfig.http_port) + "/",
                            false);
                    ...
                    out.writeBytes("HTTP/1.0 200 \n");
                    out.writeBytes("Content-type: application/x-java-jnlp-file\n");
                    out.writeBytes("Content-Disposition: attachment; filename=kadinio.jnlp\n");
                    out.writeBytes("Content-Transfer-Encoding: binary\n");
                    out.writeBytes("Content-Length: " + jnlp.getBytes().length + "\n");
                    out.writeBytes("\n");
                    out.flush();
                ...
                else if (line.startsWith("GET /kadinio.jar")) {
                    ...
                    out.writeBytes("Content-Disposition: attachment; filename=kadinio.jar\n");
                    out.writeBytes("Content-Transfer-Encoding: binary\n");
    ...
    public String getJNLP(String url, boolean isApplet) {
        return "<?xml version='1.0' encoding='UTF-8' standalone='yes'?>" +
                ...
                "<jnlp spec='1.0+'>" +
                "<information>" +
                "    <title>Kadinio - Remote Desktop Viewer</title>" +
                "    <offline-allowed/>" +
                "</information>" +
                "<security>" +
                "   <all-permissions/>" +
                ...
                "<resources>" +
                "   <j2se version='1.5+'/>" +
                "  <jar eager='true' href='" + url + "kadinio.jar' main='true'/>" +
                "</resources>" +
                (isApplet ? "<applet-desc " +
                "	width='460'" +
                "	height='320'" +
                "	main-class='jrdesktop.mainApplet'" +
                "	name='kadinio - Remote Desktop Viewer'>" +
                "</applet-desc>"
