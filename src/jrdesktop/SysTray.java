...
public class SysTray {
    ...
    public static void updateServerStatus(byte msgType, String msg) {
        ...
        switch (msgType) {
            case Commons.SERVER_RUNNING:
                ...
                trayIcon.setToolTip("kadinio [Server running]\n" + 
            ...
            case Commons.SERVER_NOT_RUNNING:
                ...
                trayIcon.setToolTip("kadinio [Server stopped]\n" + 
                        RMIServer.serverConfig.server_address);
                break;          
            case Commons.SERVER_STARTED:
                serverItem.setLabel("Stop");
                trayIcon.displayMessage("Connection status", "Kadinio Started !!",
                        MessageType.INFO);
                trayIcon.setImage(new ImageIcon(Commons.WAIT_ICON).getImage());
                trayIcon.setToolTip("kadinio [Server running]\n" + 
                        RMIServer.serverConfig.server_address);
            ...
            case Commons.CONNECTION_FAILED:
                trayIcon.displayMessage("Connection status", "Kadinio Failed !!",
                        MessageType.ERROR);
                break;
            case Commons.SERVER_STOPPED:
                serverItem.setLabel("Start");
                trayIcon.displayMessage("Connection status", "Kadinio Stopped !!",
                        MessageType.INFO);
                trayIcon.setImage(new ImageIcon(Commons.IDLE_ICON).getImage());
                trayIcon.setToolTip("kadinio [Server stopped]\n" + 
                        RMIServer.serverConfig.server_address);
    ...
    public static void displayViewer(String viewer, int size, boolean connected) {
        ...
        if (connected) {
            trayIcon.displayMessage("Kadinio Viewer details", viewer + " connected !!",
                    MessageType.INFO);
            if (size == 0) {
                trayIcon.setImage(new ImageIcon(Commons.ALIVE_ICON).getImage());
            }
        } else {
            trayIcon.displayMessage("Kadinio Viewer details", viewer + " disconnected !!",
                    MessageType.INFO);
            if (size == 0) {
                trayIcon.setImage(new ImageIcon(Commons.WAIT_ICON).getImage());
            }
    ...
    public static void display() {
        ...
        Runnable runner = new Runnable() {
            public void run() {
                ...
                MenuItem item = new MenuItem("Open Kadinio Viewer");
