...
public class main  {
    ...
    public static void loadExtension (String args[]) throws UnknownHostException {
        Commons.init();
    ...
    public static void displayHelp(boolean full) {
        System.out.println(
            "Kadinio - Remote Desktop Viewer.");
        if (full)
            System.out.println(
            "Usage: java -jar kadinio.jar <command> [options]\n\n" + 
            ...
            "   --home:directory        kadinio home directory. \n" +
            ...
            System.out.println(
                Commons.kadinio_version + " " + Commons.kadinio_build_date);
        exit();
    }    
    ...
    public static void quit () {
        if (Settings.displayMode != Commons.DISPLAY_MODE_FRAME && !Settings.exitDisabled) {
            if (JOptionPane.showConfirmDialog(null, "kadinio is running as a " +
                ...
                "\nYou may want instead to : \n   * Close kadinio using the window's close buttom; " +
                "\n   * Disable system exit as settings tab to have a normal shutdown of kadinio" +
                "\n\nAre you sure you want to continue ?", "Warning Dialog",
                JOptionPane.YES_NO_OPTION) == JOptionPane.OK_OPTION)
                    main.exit();
        } else
            if (JOptionPane.showConfirmDialog(null, "Exit Kadinio ?", "Confirm Dialog",
